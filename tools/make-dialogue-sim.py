...
// can this choice be taken by this character/state? (identity gate + legacy flag conditions)
function choiceAvailable(char, state, ch, assumePrior){
  return matchesWhen(char, state, ch.when) && conditionsPass(state, ch.conditions, assumePrior);
}
/*</DLGSIM>*/
...
function paintChoices(){
  ...
  n.choices.forEach((ch)=>{
    const ok=choiceAvailable(char, st, ch, assume);
    if(!ok && !showLocked) return;
    shown++; const idx=++n9;
    const b=document.createElement("button"); b.className="opt"+(ok?"":" locked");
    const chk=normCheck(ch); const tag=tagFor(ch);
    let tags="";
    if(tag) tags+=`<span class="tg ${tag.cls}">${esc(tag.label)}</span>`;
    if(chk) tags+=`<span class="tg check">🎲 ${esc(chk.skill||ABBR[chk.ability]||chk.ability)}</span>`;
    let inner = (tags?`<div class="tags">${tags}</div>`:"") + `<span class="num">${idx}.</span>${esc(stripBracket(ch.text)||"(continue)")}`;
    if(chk){
      const bonus=checkBonus(char, chk, MODEL), pct=Math.round(chanceToPass(chk.dc,bonus)*100), prof=chk.skill&&isProficient(char,chk.skill,MODEL);
      inner+=`<div class="ck"><span class="chip">${chk.skill?esc(chk.skill)+' · ':''}${ABBR[chk.ability]||esc(chk.ability)} DC ${chk.dc}</span>`+
