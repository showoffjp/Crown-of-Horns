...
<div class="wrap">
 ...
 <div class="toolbar">
  ...
  <button onclick="toCombat()">⚔️ Open in Combat →</button>
...
function toCombat(){
  try{ const on={}; for(const k in model.bools) if(model.bools[k]) on[k]=true;
    localStorage.setItem("coh.combat.flags", JSON.stringify(on));
    banner("Flags staged — opening Combat; the companions will speak to this run…");
    setTimeout(()=>location.href="crown_combat.html", 350);
  }catch(e){ banner("Couldn't stage flags (storage blocked): "+e.message, true); } }
